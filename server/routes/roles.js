...
async function fetchRole(client, id) {
  ...
  const { rows: panelists } = await client.query(
    'SELECT * FROM panelists WHERE role_id = $1 ORDER BY id', [id]
  );
  const { rows: channels } = await client.query(
    'SELECT * FROM sourcing_channels WHERE role_id = $1 ORDER BY id', [id]
  );
  const { rows: approvals } = await client.query(
    'SELECT * FROM approvals WHERE role_id = $1 ORDER BY id', [id]
  );
